...
import { computeProfileCompletion } from "@/lib/match-utils";
import { getRequestStatusMeta, parseStructuredRequestMessage } from "@/lib/request-utils";
...
function toListingResponse(favorite: FavoriteListing): ListingResponse {
  return {
    id: favorite.id,
    ...
    ownerEmail: favorite.ownerEmail ?? undefined,
    ownerName: favorite.ownerName ?? null,
          ...
          <div className="lg:col-span-1">
            <div className="bg-card rounded-3xl p-6 border border-border/50 shadow-sm sticky top-24">
              ...
              {user.role === "seeker" && fullProfile && (() => {
                const pct = computeProfileCompletion(fullProfile);
                return pct < 100 ? (
                ...
                <div className="bg-card rounded-3xl p-6 border border-border/50 shadow-sm">
                  ...
                  {requests.length > 0 ? (
                    <div className="space-y-3">
                      {requests.map(r => {
                        const parsed = parseStructuredRequestMessage(r.message);
                        const status = getRequestStatusMeta(r.status);
                        return (
                            ...
                            <div className="flex items-start justify-between gap-4">
                              ...
                              <div className="mt-3 grid grid-cols-1 gap-2 text-xs text-muted-foreground sm:grid-cols-3">
                                <div className="rounded-xl border border-border px-3 py-2">
                                  <p className="font-semibold text-foreground">{t("profile.moveInDate")}</p>
                                  <p className="mt-1">{parsed.moveIn || t("listings.detail.flexible")}</p>
                                </div>
                                <div className="rounded-xl border border-border px-3 py-2">
                                  <p className="font-semibold text-foreground">{t("publicProfile.budget")}</p>
                                  <p className="mt-1">{parsed.budget || t("reports.notShared")}</p>
                                </div>
                                <div className="rounded-xl border border-border px-3 py-2">
                                  <p className="font-semibold text-foreground">{t("dashboard.nextStep")}</p>
                                  <p className="mt-1">{t(status.descriptionKey)}</p>
                                </div>
                              </div>
                            {parsed.note && (
                                ...
                                {group.items.map(r => {
                                  ...
                                  return (
                                    <div key={r.id} className="p-4">
                                      <div className="flex items-start justify-between gap-4">
                                        <div className="min-w-0 flex-1">
                                          <p className="font-medium text-sm text-foreground">{r.seekerName || parsed.requesterName || r.seekerEmail?.split("@")[0]}</p>
                                        ...
                                        {r.status === "pending" ? (
                                          <div className="flex gap-2 flex-shrink-0">
                                            <button
                                              ...
                                              title="Accept request and unlock messages"
                                            >
                                      ...
                                      <div className="mt-3 grid grid-cols-1 gap-2 text-xs text-muted-foreground sm:grid-cols-3">
                                        <div className="rounded-xl border border-border px-3 py-2">
                                          <p className="font-semibold text-foreground">{t("profile.occupation")}</p>
                                          <p className="mt-1">{parsed.occupation || t("reports.notShared")}</p>
                                        </div>
                                        <div className="rounded-xl border border-border px-3 py-2">
                                          <p className="font-semibold text-foreground">{t("profile.moveInDate")}</p>
                                          <p className="mt-1">{parsed.moveIn || t("listings.detail.flexible")}</p>
                                      ...
                                      {parsed.note && <p className="mt-3 text-sm text-muted-foreground">"{parsed.note}"</p>}
                                    </div>
                                  );
                                })}
                              </div>
