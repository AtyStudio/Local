...
export default function NewListing() {
  ...
  const [roomType, setRoomType] = useState<"private_room" | "shared_room" | "studio" | "entire_place">("private_room");
  const [preferredTenantGender, setPreferredTenantGender] = useState<"male" | "female" | "no_preference">("no_preference");
  ...
  const handleImageUpload = async (file: File, imageId: string) => {
    ...
    try {
      ...
      setImages(prev => prev.map(img => img.id === imageId ? { ...img, uploading: false, error: t("listings.uploadFailed") } : img));
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      data: {
        ...
        minStayMonths: minStayMonths ? parseInt(minStayMonths, 10) : null,
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                >
                  <option value="">{t("listings.cityPh")}</option>
                  {MOROCCAN_CITIES.map((option) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-semibold text-foreground mb-2">{t("listings.deposit")}</label>
                <input
                  type="number"
                  min="0"
                  step="100"
                  ...
                  placeholder={t("listings.depositPlaceholder")}
                  className="w-full px-4 py-3 rounded-xl bg-background border-2 border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all duration-200"
                />
              </div>

              <div>
                <label className="block text-sm font-semibold text-foreground mb-2">{t("listings.availableFrom")}</label>
                <input
                  type="date"
                  value={availableFrom}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-semibold text-foreground mb-2">{t("listings.roomType")}</label>
                <select
                  value={roomType}
                  onChange={(e) => setRoomType(e.target.value as "private_room" | "shared_room" | "studio" | "entire_place")}
                  className="w-full px-4 py-3 rounded-xl bg-background border-2 border-border text-foreground focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all duration-200"
                >
                  <option value="private_room">{t("listings.roomTypes.private_room")}</option>
                  <option value="shared_room">{t("listings.roomTypes.shared_room")}</option>
                  <option value="studio">{t("listings.roomTypes.studio")}</option>
                  <option value="entire_place">{t("listings.roomTypes.entire_place")}</option>
                </select>
              </div>

              <div>
                <label className="block text-sm font-semibold text-foreground mb-2">{t("listings.preferredTenant")}</label>
                <select
                  value={preferredTenantGender}
                  onChange={(e) => setPreferredTenantGender(e.target.value as "male" | "female" | "no_preference")}
                  className="w-full px-4 py-3 rounded-xl bg-background border-2 border-border text-foreground focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all duration-200"
                >
                  <option value="no_preference">{t("listings.tenantPreference.no_preference")}</option>
                  <option value="female">{t("listings.tenantPreference.female")}</option>
                  <option value="male">{t("listings.tenantPreference.male")}</option>
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-semibold text-foreground mb-2">{t("listings.minStay")}</label>
                <input
                  type="number"
                  min="1"
                  step="1"
                  ...
                  placeholder={t("listings.minStayPlaceholder")}
                  className="w-full px-4 py-3 rounded-xl bg-background border-2 border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all duration-200"
                />
              </div>
              ...
              <div className="flex flex-col justify-end">
                <div className="flex items-center gap-3 rounded-2xl border border-border bg-background px-4 py-3">
                  ...
                  />
                  <label htmlFor="utilitiesIncluded" className="text-sm font-medium text-foreground">
                    {t("listings.utilitiesIncluded")}
                  </label>
                </div>
            ...
            <div>
              <label className="block text-sm font-semibold text-foreground mb-2">{t("listings.houseRules")}</label>
              <textarea
                value={houseRules}
                onChange={(e) => setHouseRules(e.target.value)}
                placeholder={t("listings.houseRulesPlaceholder")}
                rows={3}
                className="w-full px-4 py-3 rounded-xl bg-background border-2 border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all duration-200 resize-none"
              />
              <p className="text-xs text-muted-foreground mt-2">
                {t("listings.houseRulesHint")}
              </p>
            </div>
