...
import { MapPin, Heart, Star, ShieldCheck, Calendar, Sofa, WalletCards } from "lucide-react";
...
import { getRoomTypeLabelKey } from "@/lib/match-utils";
...
export function ListingCard({
  ...
  const trustBadge =
    listing.isIdentityVerified && listing.isLocationVerified
      ? t("listings.card.trust.checkedListing")
      : listing.isIdentityVerified
        ? t("listings.card.trust.identityChecked")
        : t("listings.card.trust.verificationPending");
              ...
              <div className="flex items-center gap-2">
                {listing.isFeatured ? (
                  <div className="flex items-center gap-1 bg-gradient-to-r from-amber-400 to-yellow-500 text-yellow-950 px-2.5 py-1 rounded-full text-xs font-bold shadow-md">
                    <Star className="w-3 h-3 fill-yellow-950" /> Priority
                  </div>
                ) : null}
                <div
                  className={cn(
                    "flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold shadow-md backdrop-blur-md",
                    listing.isIdentityVerified ? "bg-emerald-500/90 text-white" : "bg-black/55 text-white",
                  )}
                >
                  <ShieldCheck className="w-3 h-3" /> {trustBadge}
                </div>
              </div>

              <div className="bg-background/90 backdrop-blur-md px-3 py-1.5 rounded-full text-sm font-semibold shadow-sm">
                {formatPrice(listing.price)}
                <span className="text-muted-foreground font-normal text-xs ml-1">{t("listings.card.perMonth")}</span>
              </div>
            </div>
          ...
          <div className="p-4 flex flex-col flex-grow">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <span className="inline-flex items-center gap-1 rounded-full bg-secondary px-2.5 py-1 text-[11px] font-medium text-foreground">
                ...
                {t(getRoomTypeLabelKey(listing.roomType))}
              </span>
              <span className="inline-flex items-center gap-1 rounded-full bg-secondary px-2.5 py-1 text-[11px] font-medium text-foreground">
                <WalletCards className="h-3 w-3 text-primary" />
                {listing.deposit ? t("listings.card.depositValue", { value: formatPrice(listing.deposit) }) : t("listings.card.depositNotListed")}
              </span>
              <span className="inline-flex items-center gap-1 rounded-full bg-secondary px-2.5 py-1 text-[11px] font-medium text-foreground">
                <Calendar className="h-3 w-3 text-primary" />
                {listing.availableFrom
                  ? t("listings.card.availableFrom", {
                      date: new Date(listing.availableFrom).toLocaleDateString(undefined, {
                        month: "short",
                        day: "numeric",
                      }),
                    })
                  : t("listings.card.dateFlexible")}
              </span>
            </div>

            <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-muted-foreground">
              <span className="rounded-full border border-border px-2 py-1">
                {listing.furnished ? t("listings.card.furnished") : t("listings.card.unfurnished")}
              </span>
              <span className="rounded-full border border-border px-2 py-1">
                {listing.utilitiesIncluded ? t("listings.card.utilitiesIncluded") : t("listings.card.billsSeparate")}
              </span>
              {listing.preferredTenantGender &&
                listing.preferredTenantGender !== "no_preference" && (
                  <span className="rounded-full border border-border px-2 py-1 capitalize">
                    {t(`listings.tenantPreference.${listing.preferredTenantGender}`)}
                  </span>
                )}
            </div>
