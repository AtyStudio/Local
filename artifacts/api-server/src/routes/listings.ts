...
const createListingSchema = z.object({
  ...
  roomType: z.enum(["private_room", "shared_room", "studio", "entire_place"]).optional().nullable(),
  preferredTenantGender: z.enum(["male", "female", "no_preference"]).optional().default("no_preference"),
  minStayMonths: z.number().int().positive().optional().nullable(),
  houseRules: z.string().max(400).optional().nullable(),
});
...
function formatListing(listing: ListingRow, showAnalytics = false) {
  return {
    ...
    availableFrom: listing.availableFrom ? new Date(listing.availableFrom).toISOString() : null,
    roomType: listing.roomType ?? null,
    preferredTenantGender: listing.preferredTenantGender ?? "no_preference",
    minStayMonths: listing.minStayMonths ?? null,
    houseRules: listing.houseRules ?? null,
    isIdentityVerified: listing.isIdentityVerified ?? false,
    isLocationVerified: listing.isLocationVerified ?? false,
  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        utilitiesIncluded: listingsTable.utilitiesIncluded,
        furnished: listingsTable.furnished,
        availableFrom: listingsTable.availableFrom,
        roomType: listingsTable.roomType,
        preferredTenantGender: listingsTable.preferredTenantGender,
        minStayMonths: listingsTable.minStayMonths,
        houseRules: listingsTable.houseRules,
        isIdentityVerified: listingsTable.isIdentityVerified,
        isLocationVerified: listingsTable.isLocationVerified,
  ...
  try {
    ...
    const [listing] = await db
      ...
      .values({
        ...
        deposit: result.data.deposit?.toString() ?? null,
        utilitiesIncluded: result.data.utilitiesIncluded ?? false,
        furnished: result.data.furnished ?? false,
        availableFrom: result.data.availableFrom ?? null,
        roomType: result.data.roomType ?? null,
        preferredTenantGender: result.data.preferredTenantGender ?? "no_preference",
        minStayMonths: result.data.minStayMonths ?? null,
        houseRules: result.data.houseRules ?? null,
        ownerId: user.id,
      })
      .returning();
