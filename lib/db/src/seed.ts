...
const listingsData = [
  { ownerIndex: 6, title: "Appartement lumineux quartier Maarif", description: "Bel appartement de 80m² au 3ème étage avec balcon, cuisine équipée et parking. Quartier calme et bien desservi à Casablanca.", price: "4500", city: "Casablanca", deposit: "4500", utilitiesIncluded: false, furnished: true, availableFrom: "2026-04-20", roomType: "private_room" as const, preferredTenantGender: "no_preference" as const, minStayMonths: 6, houseRules: "No smoking inside. Quiet after 10pm. Visits by appointment only.", isIdentityVerified: true, isLocationVerified: true },
  { ownerIndex: 7, title: "Appartement élégant Hassan, Rabat", description: "Appartement de 95m² avec 2 chambres, salon spacieux et terrasse privative. Vue sur les jardins. Quartier ambassades.", price: "5800", city: "Rabat", deposit: "5800", utilitiesIncluded: true, furnished: true, availableFrom: "2026-05-01", roomType: "entire_place" as const, preferredTenantGender: "no_preference" as const, minStayMonths: 12, houseRules: "Respect the residence rules and no loud gatherings.", isIdentityVerified: true, isLocationVerified: true },
  { ownerIndex: 8, title: "Appartement Fès Médina", description: "Riad traditionnel rénové en appartement moderne au cœur de la médina. Décoration authentique, patio intérieur.", price: "2200", city: "Fès", deposit: "2200", utilitiesIncluded: false, furnished: false, availableFrom: "2026-04-25", roomType: "shared_room" as const, preferredTenantGender: "male" as const, minStayMonths: 3, houseRules: "Shared kitchen. Keep common areas clean. No overnight guests.", isIdentityVerified: false, isLocationVerified: false },
  { ownerIndex: 9, title: "Studio haut standing Casablanca Centre", description: "Studio de luxe 45m² avec finitions haut de gamme, sécurité 24h/24 et salle de sport dans la résidence.", price: "6500", city: "Casablanca", deposit: "6500", utilitiesIncluded: true, furnished: true, availableFrom: "2026-04-15", roomType: "studio" as const, preferredTenantGender: "no_preference" as const, minStayMonths: 6, houseRules: "Building badge required for guests. No parties.", isIdentityVerified: true, isLocationVerified: true },
  { ownerIndex: 9, title: "Appartement moderne Tanger Détroit", description: "Appartement de 70m² avec vue sur le détroit de Gibraltar. Entièrement meublé, cuisine équipée.", price: "4200", city: "Tanger", deposit: "4200", utilitiesIncluded: false, furnished: true, availableFrom: "2026-05-10", roomType: "private_room" as const, preferredTenantGender: "female" as const, minStayMonths: 4, houseRules: "Ideal for a quiet young professional. No smoking.", isIdentityVerified: true, isLocationVerified: false },
  { ownerIndex: 10, title: "Appartement premium Anfa, Casablanca", description: "Appartement d'exception de 120m² dans la résidence premium Anfa. Piscine, gardiennage et finitions de luxe.", price: "8500", city: "Casablanca", deposit: "8500", utilitiesIncluded: true, furnished: true, availableFrom: "2026-04-18", roomType: "entire_place" as const, preferredTenantGender: "no_preference" as const, minStayMonths: 12, houseRules: "Long-term tenants preferred. Building management approval required.", isIdentityVerified: true, isLocationVerified: true },
  { ownerIndex: 11, title: "Appartement vue mer Agadir", description: "Bel appartement de 85m² avec vue panoramique sur la mer et la plage d'Agadir. Balcon spacieux, parking sécurisé.", price: "4800", city: "Agadir", deposit: "4800", utilitiesIncluded: false, furnished: false, availableFrom: "2026-06-01", roomType: "private_room" as const, preferredTenantGender: "no_preference" as const, minStayMonths: 6, houseRules: "Family building. Calm tenants only.", isIdentityVerified: false, isLocationVerified: false },
  { ownerIndex: 7, title: "Studio Gueliz, Marrakech", description: "Studio récemment rénové de 40m² au cœur de Gueliz. Accès facile aux restaurants, boutiques et administration.", price: "2800", city: "Marrakech", deposit: "2800", utilitiesIncluded: true, furnished: true, availableFrom: "2026-04-30", roomType: "studio" as const, preferredTenantGender: "no_preference" as const, minStayMonths: 3, houseRules: "Maximum one tenant. Respect neighbors and common spaces.", isIdentityVerified: true, isLocationVerified: true },
];
...
async function seed() {
  ...
  if (existingDemoUsers.length > 0) {
    ...
    await db.delete(premiumSubscriptionsTable).where(inArray(premiumSubscriptionsTable.userId, demoUserIds));
  ...
  const premiumUsers = insertedUsers.filter((user) => user.isPremium);
  if (premiumUsers.length > 0) {
    console.log("Inserting premium subscriptions...");
    await db.insert(premiumSubscriptionsTable).values(
      premiumUsers.map((user) => ({
        userId: user.id,
        paypalSubscriptionId: `demo-sub-${user.id}`,
        paypalPlanId: "demo-plan-monthly",
        status: "active" as const,
        startTime: now,
        lastPaymentAt: now,
        lastPaymentSaleId: `demo-sale-${user.id}`,
        updatedAt: now,
      })),
    );
  }
  ...
  const insertedListings = await db.insert(listingsTable).values(
    listingsData.map((l) => ({
      ...
      roomType: l.roomType,
      preferredTenantGender: l.preferredTenantGender,
      minStayMonths: l.minStayMonths,
      houseRules: l.houseRules,
      isIdentityVerified: l.isIdentityVerified,
      isLocationVerified: l.isLocationVerified,
